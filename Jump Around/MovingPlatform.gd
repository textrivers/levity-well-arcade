...
func _ready():
	...
	tween.interpolate_property(self, "follow", pos_a, pos_b, duration, Tween.TRANS_LINEAR, Tween.EASE_IN, delay)
	if wrap == false:
		tween.interpolate_property(self, "follow", pos_b, pos_a, duration, Tween.TRANS_LINEAR, Tween.EASE_IN, duration + (delay * 2))
